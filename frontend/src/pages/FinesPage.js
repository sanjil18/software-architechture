...
export default function FinesPage() {
  ...
  const statusBadge = (status) => {
    const cls  = { pending:'badge-pending', paid:'badge-paid', overdue:'badge-overdue', cancelled:'badge-cancelled' };
    const icon = { pending:'⏳', paid:'✅', overdue:'🚨', cancelled:'❌' };
    return <span className={`admin-badge ${cls[status]}`}>{icon[status]} {status.charAt(0).toUpperCase()+status.slice(1)}</span>;
  };
      ...
      <div className="table-card">
        <div className="table-header">
          ...
          <div className="table-filters">
            ...
            <button className="issue-btn-primary" style={{ padding:'8px 16px', fontSize:'0.82rem' }}
        ...
        <div style={{ overflowX:'auto' }}>
          <table>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={10} style={{ textAlign:'center', padding:'2rem', color:'var(--admin-text-muted)' }}>⏳ Loading...</td></tr>
              ) : fines.length === 0 ? (
                <tr><td colSpan={10} style={{ textAlign:'center', padding:'2rem', color:'var(--admin-text-muted)' }}>No fines found.</td></tr>
              ) : fines.map(fine => (
                <tr key={fine._id}>
                  <td className="mono" style={{ color:'var(--admin-primary)' }}>{fine.referenceNumber}</td>
                  <td>
                    <div style={{ fontWeight:500 }}>{fine.driverName}</div>
                    <div style={{ fontSize:'0.75rem', color:'var(--admin-text-muted)' }}>{fine.driverLicense}</div>
                  </td>
                  <td className="mono">{fine.vehicleNumber}</td>
                  <td style={{ fontSize:'0.82rem' }}>{fine.category?.name || fine.categoryId}</td>
                  <td>{fine.district}</td>
                  <td style={{ color:'var(--admin-warning)', fontWeight:600, fontFamily:'Space Mono,monospace', fontSize:'0.82rem' }}>
                  ...
                  <td style={{ fontSize:'0.78rem' }}>
                    {fine.paymentMethod
                      ? <span style={{ color: fine.paymentMethod==='mobile' ? 'var(--admin-accent)' : 'var(--admin-primary)' }}>
        ...
        <div className="pagination">
          <div className="pagination-info">Page {filters.page} of {pagination.pages} ({pagination.total} records)</div>
          <div className="pagination-controls">
            ...
            {Array.from({ length:Math.min(5, pagination.pages) }, (_,i) => {
              ...
              return <button key={pg} className={`page-btn ${filters.page===pg?'active':''}`}
                onClick={() => setFilters(f=>({ ...f, page:pg }))}>{pg}</button>;
            })}
