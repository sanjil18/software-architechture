...
export default function PaymentPage() {
  ...
  const handleLookup = async (e) => {
    ...
    if (!lookupForm.referenceNumber.trim() || !lookupForm.categoryId.trim()) {
      toast.error('Please enter both fields.'); return;
    }
    setLoading(true);
    try {
        ...
        <div className="container">
          <div className="steps">
            {STEP_LABELS.map((label, i) => (
              <React.Fragment key={i}>
                ...
                {i < STEP_LABELS.length - 1 && <div className={`step-line ${step > i+1 ? 'active' : ''}`} />}
              </React.Fragment>
            ))}
          </div>
          ...
          {step === 1 && (
            <div className="card">
              ...
              <p className="card-subtitle">Enter the details from your traffic fine sheet issued by the officer.</p>
              <form onSubmit={handleLookup}>
                <div className="form-group">
                  <label>Fine Reference Number</label>
                  <input type="text" placeholder="e.g., TF-2024-ABCD1234" value={lookupForm.referenceNumber}
                    onChange={(e) => setLookupForm({ ...lookupForm, referenceNumber: e.target.value.toUpperCase() })} />
                </div>
                <div className="form-group">
                  <label>Fine Category ID</label>
                  <input type="text" placeholder="e.g., TF001" value={lookupForm.categoryId}
          ...
          {step === 2 && fineData && (
            <div className="card">
              ...
              <div className="fine-detail-card">
                ...
                {[
                  ['Driver', fineData.driverName], ['License No.', fineData.driverLicense],
                  ['Vehicle No.', fineData.vehicleNumber], ['Violation', fineData.violation],
                  ['Location', fineData.location], ['District', fineData.district],
              ...
              <button className="btn btn-primary" onClick={() => setStep(3)}>✅ Confirm & Proceed to Payment</button>
              <button className="btn btn-secondary" onClick={() => { setStep(1); setFineData(null); }}>← Go Back</button>
            </div>
          )}
          ...
          {step === 3 && fineData && (
            <div className="card">
              ...
              <p className="card-subtitle">Paying LKR <strong style={{ color:'var(--gold)' }}>{fineData.amount?.toLocaleString()}</strong> for {fineData.category?.name}</p>
              <form onSubmit={handlePayment}>
                <div className="form-group">
                  <label>Cardholder Name</label>
                  <input type="text" placeholder="Name on card" value={paymentForm.cardName}
                    onChange={(e) => setPaymentForm({ ...paymentForm, cardName: e.target.value })} />
                </div>
                <div className="form-group card-num-group">
                  <label>Card Number</label>
                  <input type="text" placeholder="0000 0000 0000 0000" value={paymentForm.cardNumber} maxLength={19}
                    onChange={(e) => setPaymentForm({ ...paymentForm, cardNumber: formatCard(e.target.value) })} />
                </div>
                <div className="card-row">
                  <div className="form-group">
                    <label>Expiry Date</label>
                    <input type="text" placeholder="MM/YY" value={paymentForm.expiry} maxLength={5}
                      onChange={(e) => setPaymentForm({ ...paymentForm, expiry: formatExpiry(e.target.value) })} />
                  </div>
                  <div className="form-group">
                    <label>CVV</label>
                    <input type="password" placeholder="•••" value={paymentForm.cvv} maxLength={4}
                ...
                <div className="fine-detail-card" style={{ marginBottom:'1rem' }}>
                  <div className="fine-row"><span className="fine-row-label">Fine Amount</span><span className="fine-row-value">LKR {fineData.amount?.toLocaleString()}</span></div>
                  <div className="fine-row"><span className="fine-row-label">Processing Fee</span><span className="fine-row-value">LKR 0.00</span></div>
                ...
                <button className="btn btn-secondary" type="button" onClick={() => setStep(2)}>← Back to Review</button>
              </form>
            </div>
          )}
        </div>
