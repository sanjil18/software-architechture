...
const NAV_ITEMS = [
  ...
  { to:'/admin/issue-fine', icon:'🚨', label:'Issue Fine'  },
];
...
const PAGE_TITLES = {
  ...
  '/admin/issue-fine':   { title:'Issue Fine',   sub:'Record a new traffic violation'           },
  '/admin/edit-fine':    { title:'Edit Fine',    sub:'Update fine details'                      },
};
...
export default function Layout() {
  ...
  const pathKey = Object.keys(PAGE_TITLES).find(k => location.pathname.startsWith(k) && k !== '/admin')
    || '/admin';
  const current  = PAGE_TITLES[pathKey] || { title:'Admin Portal', sub:'' };
      ...
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          <a href="/" className="nav-item" target="_blank" rel="noreferrer">
            <span className="nav-icon">💳</span>Payment Portal
          </a>
        </nav>
